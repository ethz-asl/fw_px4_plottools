...
baro_pressure = sysvector('sensor_baro.pressure');
if (sysvector('sensor_baro.pressure').Time(1) > sysvector('differential_pressure.temperature').Time(1))
    baro_pressure = addsample(baro_pressure,...
        'Data', sysvector('sensor_baro.pressure').Data(1),...
        'Time', sysvector('differential_pressure.temperature').Time(1));
end
if (sysvector('sensor_baro.pressure').Time(end) < sysvector('differential_pressure.temperature').Time(end))
    baro_pressure = addsample(baro_pressure,...
        'Data', sysvector('sensor_baro.pressure').Data(end),...
        'Time', sysvector('differential_pressure.temperature').Time(end));
end
baro_pressure.DataInfo.Interpolation = tsdata.interpolation('zoh');
baro_pressure = resample(baro_pressure, sysvector('differential_pressure.temperature').Time);
...
air_density = (baro_pressure.Data)./...
    (287.1 .* (273.15 + sysvector('differential_pressure.temperature').Data));
airspeed_true = ...
...
airspeed_true = resample(airspeed_true, sysvector('airspeed.indicated_airspeed').Time);
airspeed_true_unfiltered = resample(airspeed_true_unfiltered, sysvector('airspeed.indicated_airspeed').Time);
