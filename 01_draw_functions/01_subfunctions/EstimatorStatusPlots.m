...
if topics.estimator_status.logged
    % innovation checkbit
    inno_check_bits = de2bi(sysvector.estimator_status_0.innovation_check_flags.Data,13);
    % NOTE: de2bi only needs an output column specification to be larger
    %       than the actual size, in which case it pads with zeros.  
    fig1 = figure();
    fig1.Name = 'Innovation Check Flags';

    titles = [string('Reject VEL NED'), ...
        string('Reject POS NE'), ...
        string('Reject POS D'),...
        string('Reject Mag X'), ...
        string('Reject Mag Y'), ...
        string('Reject Mag Z'),...
        string('Reject Yaw'), ...
        string('Reject Airspeed'), ...
        string('Reject Sideslip'),...
        string('Reject HAGL'), ...
        string('Reject Optical Flow X'), ...
        string('Reject Optical Flow Y'), ...
        string('Reject AoA')];
    
    for i = 1:13
        inno_check(i) = subplot(4,4,i);
    ...
    titles = [string('Fix Type Insufficient'), ...
        string('Number of Satellites Insufficient'), ...
        string('GDOP Insufficient'), ...
        string('Horizontal Accuracy Insufficient'),...
        string('Vertical Accuracy Insufficient'), ...
        string('Speed Accuracy Insufficient'),...
    ...
    titles = [string('Tilt Alignment Complete'), ...
        string('Yaw Alignment Complete'), ...
        string('GPS Fused'), ...
        string('Optical Flow Fused'), ...
        string('Mag Yaw Fused'),...
        string('Mag 3D Fused'), ...
        string('Mag Decl Fused'), ...
        string('Vehicle in Air'),...
        string('Wind Vel Estimated'), ...
        string('Baro Height is Primary'),...
        string('Range Finder is Primary'), ...
        string('GPS Height is Primary'),...
        string('LPOS from Ext Vision Fused'), ...
        string('Yaw from Ext Vision Fused'),...
        string('Height from Ext Vision Fused'), ...
        string('Synthetic Sideslip Fused'),...
        string('Magnetometer updates only Mag States'), ...
        string('Fixed Wing'),...
        string('Mag Fault'), ...
    ...
    % filter_fault_flags
    filter_fault_flags = de2bi(sysvector.estimator_status_0.filter_fault_flags.Data,17);
    fig4 = figure();
    fig4.Name = 'Filter Fault Flags';
    
    titles = [string('Mag X Num Error'), ...
        string('Mag y Num Error'), ...
        string('Mag Z Num Error'), ...
        string('Mag Heading Num Error'), ...
        string('Mag Decl Num Error'),...
        string('Airspeed Num Error'), ...
        string('Sideslip Num Error'), ...
        string('Opt Flow X Num Error'),...
        string('Opt Flow Y Num Error'), ...
        string('Vel N Num Error'), ...
        string('Vel E Num Error'), ...
        string('Vel D Num Error'), ...
        string('Pos N Num Error'), ...
        string('Pos E Num Error'), ...
        string('Pos D Num Error'), ...
        string('Bad Acc Bias'), ...
        string('AoA Num Error')];
    
    for i = 1:17
        filter_fault(i) = subplot(5,4,i);
    ...
    test_ratios(7) = subplot(3,3,7);
    if (isfield(sysvector.estimator_status_0, 'beta_test_ratio'))
        plot(sysvector.estimator_status_0.beta_test_ratio.Time,sysvector.estimator_status_0.beta_test_ratio.Data);
    end
    title('Sideslip Test Ratio')

    test_ratios(8) = subplot(3,3,8);
    if (isfield(sysvector.estimator_status_0, 'alpha_test_ratio'))
        plot(sysvector.estimator_status_0.alpha_test_ratio.Time,sysvector.estimator_status_0.alpha_test_ratio.Data);
    end
    title('AoA Test Ratio')
